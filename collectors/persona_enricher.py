...
def query_serper_bio(name: str, company: str) -> Dict[str, Any]:
    ...
    if config.SERPER_API_KEY:
        ...
        payload = {"q": f"{name} {company} biography education career", "num": 3}
        try:
    ...
    if not snippets and config.MONID_API_KEY:
        try:
            ...
            tf_data = query_tinyfish_search_via_monid(
                f"{name} {company} biography career education", max_results=3
            )
    ...
    return {"knowledge_graph": kg, "snippets": snippets}
...
def query_fec_person_donations(
    person_name: str, company_name: str, max_records: int = 3
) -> List[Dict[str, Any]]:
    ...
    url = (
        ...
        f"api_key={api_key}&contributor_name={encoded_name}"
...
def query_uspto_inventor_patents(person_name: str, max_patents: int = 3) -> List[Dict[str, Any]]:
    ...
    try:
        last_name = person_name.split()[-1] if person_name.split() else ""
        query_json = (
            f'{{"_contains":{{"inventor_last_name":"{last_name}"}}}}'
        )
        fields_json = '["patent_number","patent_title","patent_date"]'
        url = (
            f"https://api.patentsview.org/patents/query?q={query_json}"
            f"&f={fields_json}&o={{\"size\":{max_patents}}}"
        )
...
def query_openalex_research(person_name: str, company_name: Optional[str] = None) -> Dict[str, Any]:
    """Queries OpenAlex for scholarly publications with Affiliation Verification Gate."""
    try:
        encoded = urllib.parse.quote_plus(person_name)
        url = f"https://api.openalex.org/authors?search={encoded}&per_page=3"
        res = requests.get(url, headers={"User-Agent": "EnterpriseSalesAI/1.0"}, timeout=8)
        if res.status_code == 200:
            data = res.json()
            results = data.get("results", [])
            if results and company_name:
                comp_terms = [t.lower() for t in company_name.split() if len(t) > 3]
                for top in results:
                    affils = top.get("affiliations", []) or []
                    matched = False
                    for af in affils:
                        disp = (af.get("institution", {}).get("display_name") or "").lower()
                        if any(ct in disp for ct in comp_terms):
                            matched = True
                            break
                    if matched:
                        return {
                            ...
                            "last_known_institution": top.get("last_known_institution", {}).get(
                                "display_name"
                            ),
                        }
    except Exception:
                        ...
                        employments.append(
                            {
                                "title": emp.get("title"),
                                "employer": (
                                    emp.get("employer", {}).get("name")
                                    if isinstance(emp.get("employer"), dict)
                                    else emp.get("employer")
                                ),
                            }
                        )
                return {
                    ...
                    "skills": [
                        s.get("name") for s in entity.get("skills", []) if isinstance(s, dict)
                    ][:5],
                }
    except Exception:
        pass
...
def query_openai_dossier_synthesis(
    ...
    """
    if not config.LLM_API_KEY or not (combined_text or "").strip():
        return None
    ...
    if degree or institution:
        background_bits.append(
            f"Education: {(degree or '').strip()} {(institution or '').strip()}".strip()
        )
    ...
    try:
        ...
        if res.status_code != 200:
            print(
                f"[!] [PersonaEnricher] AI dossier synthesis HTTP {res.status_code}: {res.text[:200]}"
            )
            return None
        ...
        if not required_keys.issubset(parsed.keys()):
            print(
                f"[!] [PersonaEnricher] OpenAI dossier response missing expected keys: {list(parsed.keys())}"
            )
            return None
        return parsed
    except Exception as e:
...
def build_persona_dossier(
    ...
    **kwargs,
    ...
    Delegates to unified PersonaService for FullEnrich contact waterfall,
    Apify LinkedIn career history, OpenAlex academic background, and AI dossier synthesis.
    """
    ...
    print(
        f"[*] [Ultimate Persona Enricher] Synthesizing multi-source AI dossier for "
        f"'{actual_name}' ({actual_title}, {actual_company})..."
    )

    try:
        from services.persona_service import PersonaService

        dossier = PersonaService.enrich_single_persona(
            full_name=actual_name,
            company_name=actual_company,
            title=actual_title,
            linkedin_url=actual_linkedin,
            account_id=kwargs.get("account_id"),
            lob_id=kwargs.get("lob_id"),
            domain=kwargs.get("domain"),
            run_raw_dir=kwargs.get("run_raw_dir"),
        )
        if dossier:
            raw_exa = kwargs.get("raw_exa_dir")
            raw_srp = kwargs.get("raw_serper_dir")
            slug = re.sub(r"[^a-z0-9]+", "_", actual_name.lower()).strip("_")
            raw_payload = dossier.get("raw_data", {})
            if raw_exa and raw_payload.get("exa"):
                try:
                    Path(raw_exa).mkdir(parents=True, exist_ok=True)
                    with open(Path(raw_exa) / f"{slug}_exa_raw.json", "w", encoding="utf-8") as f:
                        json.dump(raw_payload["exa"], f, indent=2, ensure_ascii=False)
                except Exception:
                    pass
            if raw_srp and raw_payload.get("serper"):
                try:
                    Path(raw_srp).mkdir(parents=True, exist_ok=True)
                    with open(Path(raw_srp) / f"{slug}_serper_raw.json", "w", encoding="utf-8") as f:
                        json.dump(raw_payload["serper"], f, indent=2, ensure_ascii=False)
                except Exception:
                    pass
            return dossier
    except Exception as e:
        print(
            f"[!] [PersonaEnricher] PersonaService notice: {e}. Falling back to standard dossier builder..."
        )
    ...
    combined_text = (
        f"{exa_data.get('text', '')} "
        f"{' '.join(serper_data.get('snippets', []))} "
        f"{' '.join(tavily_data.get('results', []))}"
    )
    ...
    degree_match = re.search(
        r"\b(B\.?S\.?|B\.?A\.?|M\.?S\.?|M\.?B\.?A\.?|Ph\.?D\.?|Bachelor|Master|Doctorate)\b",
        combined_text,
        re.IGNORECASE,
    )
    if degree_match:
    ...
    inst_match = re.search(
        r"\b(Harvard|Stanford|MIT|Columbia|Wharton|Yale|Princeton|Oxford|Cambridge|"
        r"Cornell|Dartmouth|NYU|Berkeley|Northwestern|University of [A-Za-z]+)\b",
        combined_text,
        re.IGNORECASE,
    )
    if inst_match:
    ...
    prior_match = re.search(
        r"(?:previously at|former|prior to joining|ex-|worked at)\s+([A-Z][A-Za-z0-9\s&]+?)(?:\.|\,|as|in|where)",
        combined_text,
        re.IGNORECASE,
    )
    if prior_match:
    ...
    resolved_linkedin = (
        ...
        or exa_data.get("url")
        or (
            f"https://www.linkedin.com/in/{actual_name.lower().replace(' ', '')}"
            if not actual_linkedin
            else actual_linkedin
        )
    )
    ...
    if llm_dossier:
        ...
        if "ceo" in title_lower or "chief executive" in title_lower or "president" in title_lower:
            kpis = [
                ...
                "Enterprise Valuation & M&A",
                "Customer Retention",
            ]
            pain_points = [
                "Macroeconomic volatility",
                "Legacy technology modernization debt",
                "Market competition & fee compression",
            ]
        elif "cio" in title_lower or "cto" in title_lower or "technology" in title_lower:
            kpis = [
                "Infrastructure Uptime & SLA",
                "Cloud Modernization & Security Compliance",
                "GenAI & Automation Adoption",
            ]
            pain_points = [
                "Technical debt in core processing systems",
                "Cybersecurity compliance friction",
                "Talent retention",
            ]
            ...
            pain_points = [
                "Cost inflation across vendor contracts",
                "Regulatory compliance reporting overhead",
                "Billing reconciliation",
            ]
        else:
            kpis = [
                "Departmental Delivery & Efficiency",
                "Team Velocity & Talent Retention",
                "Budgetary Governance",
            ]
            pain_points = [
                "Cross-functional communication friction",
                "Process bottlenecks & manual overhead",
            ]
        ...
        value_proposition = (
            f"Empowering {actual_company}'s {actual_title} with AI-driven operational "
            "scaling and automated efficiency"
        )
        key_objections = [
        ...
        prior_str = f" following your background at {prior_company}" if prior_company else ""
        personalized_icebreaker = (
            f"Noticed your strategic leadership driving initiatives at "
            f"{actual_company}{prior_str}."
        )
    ...
    return {
        ...
        "level_1_demographics": {
            ...
            "scholarly_metrics": (
                openalex_research if openalex_research.get("works_count") else None
            ),
        },
        ...
        "level_3_personal_touch": {
            ...
            "social_media": {"profile_url": resolved_linkedin, "verified": bool(resolved_linkedin)},
        },
    }
