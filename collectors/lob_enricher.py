...
def query_tavily_lob_financials(
    lob_name: str, parent_name: str, raw_tavily_dir: Optional[Path] = None
) -> Dict[str, Any]:
            ...
            if res.status_code == 200:
                ...
                if raw_tavily_dir:
                    try:
                        ...
                        lob_slug = slugify(lob_name)
                        out_path = raw_tavily_dir / f"{lob_slug}_tavily_financials_raw.json"
                        with open(out_path, "w", encoding="utf-8") as f:
                            json.dump(data, f, indent=2, ensure_ascii=False)
                    except Exception as save_err:
                        print(f"[!] [LOBEnricher] Tavily save error: {save_err}")
    ...
    return {"financial_snippets": snippets, "sources": sources}
        ...
        if res.status_code == 200:
            ...
            if items:
                ...
                return {
                    ...
                    "competitors": [
                        c.get("name") for c in entity.get("competitors", []) if isinstance(c, dict)
                    ],
...
def query_gleif_lob_lei(lob_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        res = requests.get(
            url,
            headers={"Accept": "application/vnd.api+json", "User-Agent": "EnterpriseSalesAI/1.0"},
            timeout=10,
        )
        if res.status_code == 200:
            data = res.json()
...
def query_uspto_lob_patents(lob_name: str, max_patents: int = 5) -> List[Dict[str, Any]]:
    ...
    try:
        q_json = f'{{"_contains":{{"assignee_organization":"{lob_name}"}}}}'
        f_json = '["patent_number","patent_title","patent_date"]'
        patentsview_url = (
            f"https://api.patentsview.org/patents/query?q={q_json}"
            f"&f={f_json}&o={{\"size\":{max_patents}}}"
        )
...
def query_wikipedia_lob_summary(lob_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        res = requests.get(
            ...
            headers={"User-Agent": "EnterpriseSalesAI/1.0"},
            timeout=8,
        )
        if res.status_code == 200:
            data = res.json()
...
def extract_dynamic_operating_head(text: str) -> Optional[str]:
    ...
    patterns = [
        ...
        r"([A-Z][a-z]+(?:\s+[A-Z][a-z]+)+)\s*,\s*(?:Global Head|Head|President|CEO|COO|Managing Director)",
...
def generate_generic_lob_overview(
    lob_name: str, parent_name: str, jurisdiction: Optional[str] = None
) -> str:
    ...
    return (
        ...
        f"of {parent_name}{jur_text}, providing focused commercial, operational, "
        "and institutional solutions."
    )


def enrich_lob_segments(
    company_name: str,
    sublobs: List[Dict[str, Any]],
    raw_tavily_dir: Optional[Path] = None,
    run_raw_dir: Optional[Path] = None,
) -> List[Dict[str, Any]]:
    """
    Ultimate Multi-Source LOB Enricher.
    Delegates to LobService for unified multi-source enrichment across UK Companies House,
    Wappalyzer, GLEIF, Patents, and Wikipedia.
    100% Dynamic, Zero Hardcoding.
    """
    print(
        f"[*] [Ultimate LOB Enricher] Elevating {len(sublobs)} LOBs for '{company_name}' via LobService..."
    )
    try:
        from services.lob_service import LobService

        return LobService.enrich_all_lobs(
            lobs_list=sublobs, parent_company=company_name, run_raw_dir=run_raw_dir
        )
    except Exception as e:
        print(
            f"[!] [LOBEnricher] LobService notice: {e}. Falling back to standard enrichment loop..."
        )

    enriched = []
    for lob in sublobs:
        name = lob.get("name") or lob.get("lob_name", "Division")
        ...
        final_overview = (
            overview
            or wiki_intel.get("extract")
            or diffbot_intel.get("description")
            or generate_generic_lob_overview(name, company_name, jurisdiction)
        )
        ...
        audited_rev = (
            lob.get("audited_segment_revenue")
            or diffbot_intel.get("yearly_revenue")
            or extract_dynamic_segment_revenue(joined_snippets)
        )
        ...
        entry = {
            ...
            or diffbot_intel.get("employee_count"),
