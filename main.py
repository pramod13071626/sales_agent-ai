...
def run_pipeline(company_name: str, target_url: str = None):
    ...
    try:
        ...
        account_data = scrape_account(
            ...
            raw_serper_dir=run_dirs["raw_serper_dir"],
        )
        telemetry.record_api_call(
            "apify", "crunchbase-companies-scraper", credits=1, is_billable=True
        )
        safe_nm = run_dirs["safe_name"]
        if (run_dirs["raw_apify_dir"] / f"{safe_nm}_account_linkedin_raw.json").exists():
            telemetry.record_api_call(
                "apify", "linkedin-company", credits=1, is_billable=True
            )
        if (run_dirs["raw_serper_dir"] / f"{safe_nm}_crunchbase_discovery_serper_raw.json").exists():
            telemetry.record_api_call(
                "serper", "google.serper.dev/search", credits=1, is_billable=True
            )
        sec_cik = account_data.get("sec_cik")

        # Multi-source account enhancements
        diffbot_intel = fetch_diffbot_organization_intel(
            company_name, target_url, raw_dir=run_dirs["raw_diffbot_dir"]
        )
        if diffbot_intel.get("status") == "success":
            account_data["diffbot_intel"] = diffbot_intel
            telemetry.record_api_call(
                "diffbot", "/kg/v3/enhance?type=Organization", credits=1, is_billable=True
            )

        gleif_tree = fetch_gleif_ownership_tree(
            company_name, raw_dir=run_dirs["raw_gleif_dir"]
        )
        ...
        if sec_cik:
            ...
            if sec_10k.get("status") == "success":
                ...
                telemetry.record_api_call(
                    "sec_edgar", "/submissions/CIK.json", credits=0, is_billable=False
                )
        ...
        if patents_data.get("status") == "success":
            ...
            telemetry.record_api_call(
                "uspto_data_gov", "api.patentsview.org", credits=0, is_billable=False
            )

        wiki_data = fetch_wikipedia_dbpedia_intel(
            company_name, raw_dir=run_dirs["raw_wikipedia_dir"]
        )
        if wiki_data.get("status") == "success":
            account_data["wikipedia_intel"] = wiki_data
            telemetry.record_api_call(
                "wikipedia", "en.wikipedia.org/api/rest_v1", credits=0, is_billable=False
            )
        ...
        if fec_data.get("status") == "success":
            ...
            telemetry.record_api_call(
                "openfec_data_gov", "api.open.fec.gov", credits=0, is_billable=False
            )
        ...
        telemetry.log(
            "INFO",
            "LOBS",
            "Discovering subsidiaries across Crunchbase, SEC EX-21, and GLEIF...",
        )
        sublobs_raw = scrape_sublobs(
            parent_account_name=company_name, raw_apify_dir=run_dirs["raw_apify_dir"]
        )
        telemetry.record_api_call(
            "apify", "crunchbase-suborganizations", credits=1, is_billable=True
        )

        if sec_cik:
            ex21 = fetch_sec_exhibit_21_subsidiaries(
                sec_cik, raw_sec_dir=run_dirs["raw_sec_dir"]
            )
            if ex21.get("status") == "success":
                telemetry.record_api_call(
                    "sec_edgar", "Form 10-K EX-21", credits=0, is_billable=False
                )
                        ...
                        sublobs_raw.append(
                            {
                                "name": l_name,
                                "relationship_type": (
                                    f"Legal Subsidiary ({legal_sub.get('jurisdiction', 'US')})"
                                ),
        ...
        sublobs_data = enrich_lob_segments(
            company_name,
            clustered_subs,
            raw_tavily_dir=run_dirs["raw_tavily_dir"],
            run_raw_dir=run_dirs["raw_dir"],
        )
        telemetry.record_api_call(
            "tavily", "/search?depth=financials", credits=len(sublobs_data), is_billable=True
        )
        ...
        telemetry.log(
            "INFO",
            "HIERARCHY",
            "Extracting 4-tier management and board leadership...",
        )
        account_domain = (
            account_data.get("domain") or account_data.get("primary_domain") or target_url
        )
        ...
        if c_suite:
            for exec_person in c_suite[:7]:
                ...
                exec_person["persona_dossier"] = build_persona_dossier(
                    ...
                    run_raw_dir=run_dirs["raw_dir"],
                    raw_exa_dir=run_dirs["raw_exa_dir"],
                    raw_serper_dir=run_dirs["raw_serper_dir"],
                )
                telemetry.record_api_call(
                    "exa", "/search?category=people", credits=1, is_billable=True
                )
                if (
                    ...
                    verified_url = exec_person["persona_dossier"]["level_3_personal_touch"][
                        "social_media"
                    ]["profile_url"]
        ...
        for lob in sublobs_data:
            ...
            if lob_name:
                print(
                    f"[*] Extracting dedicated hierarchy for LOB: '{lob_name}' "
                    f"(domain: {lob_domain or 'N/A'})..."
                )
                lob_res = scrape_lob_hierarchy(
                    lob_name=lob_name,
                ...
                lob_hier = lob_res.get("hierarchy", lob_res)
                telemetry.record_api_call(
                    "apollo_monid",
                    f"/v1/people/search?query={lob_name}",
                    credits=1,
                    is_billable=True,
                )
            else:
        ...
        telemetry.log(
            "INFO", "SERIALIZATION", f"Generating company-namespaced slices for '{company_name}'..."
        )
        ...
        telemetry.set_quality_audit(
            score=meta["overall_quality_score"], grade=meta["quality_grade"]
        )
    ...
    try:
        ...
        session.commit()
        print(
            f"[+] [RunManager] Staged Run Record committed to PostgreSQL 'pipeline_runs' table (ID: {run_id})"
        )
    except Exception as e:
        session.rollback()
    ...
    print("=" * 70)
    print(
        f"[INFO] Quality Score: {telemetry.quality_score}/100 "
    ...
    return {"run_dirs": run_dirs, "validation_report": validation_report}


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Sales AI Intelligence Pipeline")
    parser.add_argument(
        "--name", "-n", type=str, help="Target company name (e.g. 'BNY', 'BlackRock')"
    )
    parser.add_argument(
        "--url", "-u", type=str, help="Target company website URL (e.g. 'https://www.bny.com')"
    )
    parser.add_argument(
        "--validate", type=str, help="Validate an existing run directory or enriched JSON file"
    )
    parser.add_argument(
        "--dump-db",
        type=str,
        help="Dump a validated run directory or enriched JSON file to PostgreSQL",
    )
    ...
    if args.validate:
        ...
        if not target_url:
            target_url = input(
                "Enter target company website URL (optional, press Enter to auto-discover): "
            ).strip()
            if not target_url:
                target_url = None
