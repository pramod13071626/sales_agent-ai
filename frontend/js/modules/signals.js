...
export function renderEngagementPanel(account) {
  ...
  if (!rows.length) {
    return `<div class="empty-block">
      <div class="empty-block-icon"><i class="fa-solid fa-arrow-trend-down"></i></div>
...
export function computeSignals(account, lob) {
  ...
  if (msi.linkedin_metrics && msi.linkedin_metrics.follower_count) {
    ...
    signals.push({
      icon: 'fa-brands fa-linkedin', text: `${Number(lm.follower_count).toLocaleString()} LinkedIn followers`,
      ...
      detail: `
        ...
        ${account.linkedin_url ? `<a class="social-link" style="margin-top:10px;" href="${esc(account.linkedin_url)}" target="_blank"><i class="fa-solid fa-arrow-up-right-from-square"></i> Open LinkedIn profile</a>` : ''}
      `
    });
  }
  if (msi.linkedin_metrics && msi.linkedin_metrics.exact_employee_headcount) {
    signals.push({ icon: 'fa-solid fa-users', text: `${Number(msi.linkedin_metrics.exact_employee_headcount).toLocaleString()} employees (verified via LinkedIn)` });
  }
  if (msi.sec_10k_chunks_meta && (msi.sec_10k_chunks_meta.sections_found || []).length) {
    const meta = msi.sec_10k_chunks_meta;
    signals.push({
      icon: 'fa-solid fa-file-lines', text: `Recent 10-K on file — ${meta.sections_found.join(', ')}`,
      ...
      detail: `
        ...
        <button type="button" class="action-btn" id="fetchSecBtn" data-cik="${esc(account.sec_cik || '')}" style="margin-top:12px;">
          <i class="fa-solid fa-cloud-arrow-down"></i> Fetch full filing text
        </button>
  ...
  if (msi.gleif_intel && msi.gleif_intel.lei_code) {
    signals.push({
      icon: 'fa-solid fa-circle-check', text: `LEI verified: ${msi.gleif_intel.lei_code} (${msi.gleif_intel.jurisdiction || 'jurisdiction unknown'})`,
      ...
      detail: `
        ...
        <a class="social-link" style="margin-top:10px;" href="https://search.gleif.org/#/record/${encodeURIComponent(msi.gleif_intel.lei_code)}" target="_blank"><i class="fa-solid fa-arrow-up-right-from-square"></i> View on GLEIF registry</a>
      `
    });
  }
  ...
  if (account.company_type === 'Public') {
    signals.push({
      icon: 'fa-solid fa-arrow-trend-up', text: `Publicly traded — ${account.ticker || ''} on ${account.stock_exchange || 'a public exchange'}`,
  ...
  if (account.last_funding_type) {
    ...
    signals.push({
      icon: 'fa-solid fa-coins', text: `Last funding: ${account.last_funding_type}${date}${account.total_funding_amount_usd ? ` — ${amt}` : ''}`,
  ...
  if (account.ipo_status) {
    signals.push({
      icon: 'fa-solid fa-building-columns', text: `IPO status: ${account.ipo_status}`,
      title: 'IPO Status',
      detail: `
  ...
  if (account.patents_granted) {
    signals.push({ icon: 'fa-solid fa-lightbulb', text: `${account.patents_granted} patent${account.patents_granted !== 1 ? 's' : ''} granted` });
  }
  if (account.active_tech_count) {
    const techSet = new Set();
    (account.lobs || []).forEach(l => (l.technologies || []).forEach(t => techSet.add(t)));
    signals.push({
      icon: 'fa-solid fa-microchip', text: `${account.active_tech_count} active technologies detected${account.it_spend ? ` (IT spend: ${account.it_spend})` : ''}`,
      ...
      detail: `
        ...
        ${techSet.size ? `<div class="dossier-label" style="margin-top:10px;">Detected across LOBs</div><div class="chip-row">${[...techSet].map(t => `<span class="chip"><i class="fa-solid fa-microchip"></i> ${esc(t)}</span>`).join('')}</div>` : ''}
      `
    });
  }
  if (account.num_acquisitions) {
    signals.push({ icon: 'fa-solid fa-briefcase', text: `${account.num_acquisitions} acquisition${account.num_acquisitions !== 1 ? 's' : ''} on record` });
  }
  if ((account.lobs || []).length > 1) {
    signals.push({
      icon: 'fa-solid fa-folder', text: `${account.lobs.length} active lines of business tracked`,
  ...
  if (account.total_contacts_captured) {
    ...
    signals.push({
      icon: 'fa-solid fa-users', text: `${account.total_contacts_captured} contacts mapped across the org`,
      title: 'Org Coverage',
      detail: `
  ...
  if ((account.industries || []).length) {
    signals.push({
      icon: 'fa-solid fa-tag', text: `Operates in ${account.industries.slice(0, 3).join(', ')}`,
      title: 'Industries',
      detail: `<div class="chip-row">${account.industries.map(i => `<span class="chip"><i class="fa-solid fa-tag"></i> ${esc(i)}</span>`).join('')}</div>`
    });
  }
  ...
  if (competitors.size) {
    signals.push({
      icon: 'fa-solid fa-shield-halved', text: `Competitors tracked: ${[...competitors].slice(0, 3).join(', ')}${competitors.size > 3 ? ` +${competitors.size - 3} more` : ''}`,
      title: 'Competitive Landscape',
      detail: `<div class="chip-row">${[...competitors].map(c => `<span class="chip"><i class="fa-solid fa-shield-halved"></i> ${esc(c)}</span>`).join('')}</div>`
    });
  }
  return signals;
}
